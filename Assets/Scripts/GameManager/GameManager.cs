...
public class GameManager : SingletonMonobehaviour<GameManager>
{
    ...
    protected override void Awake()
    {
        ...
        //gameState = GameState.textureGeneration; // PAUSE GAME AWAL
    ...
    private void Start()
    {
    currentDungeonLevelListIndex = PlayerPrefs.GetInt("lastLevel", 0);

    if (currentDungeonLevelListIndex == 0) // Level 1 (index 0)
    {
        Debug.Log("Starting game at Level 1 without texture generation.");
        //textureManager.ApplyDefaultTexturesToAllCachedRenderers();
    ...
    {
        Debug.Log($"Starting game at Level {currentDungeonLevelListIndex + 1} with texture generation.");
        gameState = GameState.textureGeneration;
        ...
        if (uiManager != null)
        {
            uiManager.StartTextureGenerationUI();
        }

        Time.timeScale = 0f; // Pause game selama texture generation
    }
        // Set score to zero
        gameScore = 0;
    ...
    private void HandleGameState()
    {
        ...
        switch (gameState)
        {
            case GameState.textureGeneration:
            return;
    ...
    private IEnumerator DisplayDungeonLevelText()
    {
        ...
        yield return StartCoroutine(DisplayMessageRoutine("PRESS WASD OR ARROW KEY TO MOVE\n\nCLICK LEFT MOUSE TO SHOOT AND CLICK RIGHT MOUSE TO SPRINT\n\nYOU'RE IMMUNE WHEN SPRINTING", Color.white, 5f));

        yield return StartCoroutine(DisplayMessageRoutine("PRESS E TO LOOT CHEST", Color.white, 5f));
        // Fade out canvas
    ...
    private IEnumerator LevelCompleted()
    {
        ...
        yield return StartCoroutine(DisplayMessageRoutine("COLLECT ANY LOOT ....THEN PRESS ENTER\n\nTO DESCEND FURTHER INTO THE DUNGEON", Color.white, 5f));
    ...
    private IEnumerator GameLost()
    {
        ...
        yield return StartCoroutine(DisplayMessageRoutine("PRESS ENTER TO GO BACK TO MAIN MENU", Color.white, 0f));
    ...
    public DungeonLevelSO GetCurrentDungeonLevel()
    {
        ...
        public void StartGameAfterTextureGeneration()
    {
        if (gameState == GameState.textureGeneration)
        {
            gameState = GameState.gameStarted;
            Time.timeScale = 1f; // Unfreeze game
            GetPlayer().playerControl.EnablePlayer();
            // Mulai game (hanya jika belum started)
            if (previousGameState != GameState.gameStarted)
            {
                PlayDungeonLevel(currentDungeonLevelListIndex);
            }
        }
        ...
        public void ReturnToTextureGeneration()
    {
        Debug.Log("Returning to Texture Generation Phase...");

        // Pause game time for texture generation
        Time.timeScale = 0f;
        ...
        // Safety checking before memanggil UI
        if (UIManager.Instance != null)
        {
            UIManager.Instance.StartTextureGenerationUI();
        }
        else
        {
            Debug.LogError("ReturnToTextureGeneration: UIManager instance is null");
        }
    }
